...
from regressor_models_exp1 import LSTMModel, TransformerModel, CNNLSTMModel, CNNModel
...
pdf_filename = f"/home/rxb2495/glucose_predictions_all_subjects_plots_DL_{current_time}.pdf"
pdf_pages = PdfPages(pdf_filename)

# CSV file to save actual vs predicted values
csv_filename = f"/home/rxb2495/glucose_predictions_values_DL_{current_time}.csv"
csv_data = []
...
def pipeline_run(intervals, output_data, m_epochs, model_results, regression_config, id_, index):
    """
    ...
    for model_name, (model_class, model_params) in deep_models.items():
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(intervals)):
            ...
            for actual, predicted in zip(test_true, test_pred):
                csv_data.append({
                    "Model": model_name,
                    "Fold": fold + 1,
                    "Actual": actual,
...
list_of_features = [features_1, features_2]
...
for index, features in enumerate(list_of_features):
    ...
    for interval_split in [96]:
        ...
        for id_ in ids:
            ...
            model_results = pipeline_run(intervals, output_data, m_epochs, model_results, regression_config, id_, index)
